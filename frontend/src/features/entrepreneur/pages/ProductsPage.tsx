...
export function ProductsPage() {
  ...
  // Estado para confirmación de eliminación y Toast
  ...
  const handleSave = () => {
    if (editingProduct) {
      ...
      setToast({ show: true, message: 'Producto creado correctamente.', type: 'success' })
    }
    setModalOpen(false)
  }
  ...
  const handleConfirmDelete = () => {
    if (targetId) {
      setProductList((prev) => prev.filter((p) => p.id !== targetId))
      setToast({ show: true, message: 'Producto eliminado correctamente.', type: 'success' })
      setTargetId(null)
    }
  }
  ...
  const togglePublish = (id: string) => {
    ...
    if (p) {
      setToast({
        show: true,
        message: !p.published ? 'Producto publicado correctamente.' : 'Producto despublicado correctamente.',
        type: 'success',
      })
    }
  }
            ...
            <Card key={product.id} padding={false} className="overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => requestDelete(product.id)}>
